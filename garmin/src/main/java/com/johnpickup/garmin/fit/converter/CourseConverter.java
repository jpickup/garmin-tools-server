package com.johnpickup.garmin.fit.converter;

import com.johnpickup.garmin.fit.route.Course;
import com.johnpickup.garmin.fit.route.CoursePoint;
...
public class CourseConverter {
    public Course convert(GpxType gpx) {
        ...
        if (!gpx.getTrk().isEmpty()) {
            return convertTrack(gpx.getTrk().getFirst(), alternativeName);
        }

        if (!gpx.getRte().isEmpty()) {
            return convertRoute(gpx.getRte().getFirst(), alternativeName);
        }

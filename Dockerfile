...
FROM --platform=linux/amd64 docker.io/library/maven:3.9.9-eclipse-temurin-23 AS dependencies
WORKDIR /opt/app
...
# build the jar
FROM --platform=linux/amd64 docker.io/library/maven:3.9.9-eclipse-temurin-23 AS builder
WORKDIR /opt/app
...
FROM --platform=linux/amd64 docker.io/library/eclipse-temurin:23
